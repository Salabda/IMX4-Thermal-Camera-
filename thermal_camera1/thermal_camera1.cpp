...
void logFloatsToMatrixBin(float* pTempMatrix, int dataSize) {
	...
	if (timeDiffMatrix.count() >= minTimeIntervalMsMatrix) {
		if (TemperatureMatrixFile.is_open()) {
			...
			TemperatureMatrixFile.write(reinterpret_cast<const char*>(pTempMatrix), dataSize * sizeof(float));

			// Add clear spacing between chunks.
			for (int i = 0; i < spacing; ++i) {
				TemperatureMatrixFile.write(reinterpret_cast<const char*>(&i), sizeof(float));
			}
